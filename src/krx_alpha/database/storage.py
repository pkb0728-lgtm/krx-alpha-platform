...
def decision_journal_csv_path(project_root: Path, journal_name: str = "decision_journal") -> Path:
    dataset_dir = signals_dataset_dir(project_root, "decision_journal")
    return dataset_dir / f"{journal_name}.csv"


def decision_journal_evaluation_file_path(
    project_root: Path,
    holding_days: int,
    journal_name: str = "decision_journal",
) -> Path:
    dataset_dir = signals_dataset_dir(project_root, "decision_journal_evaluation")
    return dataset_dir / f"{journal_name}_h{holding_days}.parquet"


def decision_journal_evaluation_csv_path(
    project_root: Path,
    holding_days: int,
    journal_name: str = "decision_journal",
) -> Path:
    dataset_dir = signals_dataset_dir(project_root, "decision_journal_evaluation")
    return dataset_dir / f"{journal_name}_h{holding_days}.csv"


def decision_journal_report_file_path(
    project_root: Path,
    holding_days: int,
    journal_name: str = "decision_journal",
) -> Path:
    return project_root / "reports" / "decision_journal" / f"{journal_name}_h{holding_days}.md"
