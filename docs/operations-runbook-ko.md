...
1. `오늘 결론부터 보기`: 전체 결과를 초보자용 요약으로 확인
2. `판단 성과 추적`: 과거 판단이 이후 실제 수익률과 맞았는지 확인
3. `유니버스 순위`: 종목들이 성공적으로 처리됐는지 확인
4. `자동 스크리너`: 통과 후보와 보류 사유 확인
5. `KIS 모의투자 검토 후보`: KIS 모의투자 후보와 `실제 주문 수 = 0` 확인
6. `페이퍼 포트폴리오`: 가상 포트폴리오 거래/현금/노출 확인
7. `운영 상태`: 산출물이 정상인지 확인
